...
function getSampleTasks(){
  ...
  const sample=true;
  return [
    {id:s.welcome,title:'Welcome to Do-able!',description:'This is your first task. Edit, tag, annotate, or mark it done to explore the app.',status:'done',priority:'medium',due_date:yestStr,category:'Getting Started',tags:['welcome','tutorial'],sample,annotations:[{id:sid(),text:'Checked the detail page — nice layout.',timestamp:nowISO()}],created_at:d2agoStr,updated_at:nowISO(),deleted_at:null},
    {id:s.homepage,title:'Design homepage mockup',description:'Create wireframes for the landing page refresh. Include hero, feature grid, testimonial carousel.',status:'in_progress',priority:'high',due_date:futStr,category:'Design',tags:['design','frontend','mockup'],sample,annotations:[{id:sid(),text:'Draft v1 shared in Figma',timestamp:nowISO()},{id:sid(),text:'Client wants more whitespace',timestamp:nowISO()}],created_at:lastWkStr+'T09:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.loginBug,title:'Fix login bug (Safari 403)',description:'Safari users get a 403 on /auth/login. Cookie SameSite issue suspected.',status:'not_started',priority:'high',due_date:todayStr(),category:'Development',tags:['bug','urgent','backend','security'],sample,annotations:[{id:sid(),text:'Reproduced in Safari 18 on macOS; Chrome + Firefox unaffected.',timestamp:nowISO()},{id:sid(),text:'Root cause: SameSite=None missing Secure flag on dev cert',timestamp:nowISO()}],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.weeklyReview,title:'Weekly review notes',description:'Prepare agenda for Friday team sync. Discuss sprint progress and blockers.',status:'not_started',priority:'medium',due_date:futStr,category:'Meetings',tags:['recurring','team','sync'],sample,recur:'weekly',annotations:[],created_at:lastWkStr+'T14:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.depReport,title:'Deprecated draft report',description:'Q1 analysis draft — superseded by the final version.',status:'done',priority:'low',due_date:'',category:'Reports',tags:['old','archive'],sample,annotations:[],created_at:lastMthStr,updated_at:lastMthStr,deleted_at:d2agoStr},
    {id:s.deploy,title:'Deploy v2 to staging',description:'Push the current feature branch to staging for QA. Needs mockup sign-off first.',status:'not_started',priority:'high',due_date:futStr,category:'Development',tags:['deploy','devops'],sample,depends_on:[s.homepage],annotations:[{id:sid(),text:'Coordinated with DevOps — they are ready for Friday',timestamp:nowISO()}],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.morningJog,title:'Morning jog',description:'30 min run before breakfast. Loop around the park.',status:'done',priority:'medium',due_date:yestStr,start_date:yestStr,time:'06:30',category:'Health',tags:['exercise','routine'],sample,recur:'daily',annotations:[{id:sid(),text:'5.2 km in 28 min — new PR!',timestamp:nowISO()}],created_at:lastWkStr+'T06:30:00',updated_at:nowISO(),deleted_at:null},
    {id:s.deepRead,title:'Read "Deep Work"',description:'Finish chapters 5-7 this week. Take notes on the four deep work philosophies.',status:'in_progress',priority:'low',due_date:nextWkStr,category:'Personal',tags:['reading','learning','focus'],sample,annotations:[{id:sid(),text:'Ch5: Monastic vs Bimodal — I lean bimodal',timestamp:nowISO()}],created_at:lastWkStr+'T20:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.budget,title:'Quarterly budget review',description:'Review Q2 spending against plan. Flag any line items over 10% of budget.',status:'not_started',priority:'high',due_date:nextWkStr,category:'Finance',tags:['budget','planning','review'],sample,annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.apiDocs,title:'Write API docs',description:'Document the new REST endpoints for the team. Include request/response examples.',status:'done',priority:'medium',due_date:lastWkStr,category:'Development',tags:['docs','api','backend'],sample,annotations:[{id:sid(),text:'Published to internal wiki — link in #dev channel',timestamp:nowISO()}],created_at:lastMthStr,updated_at:lastWkStr+'T17:30:00',deleted_at:null},
    {id:s.errorHandling,title:'Update error handling middleware',description:'Add global try-catch wrapper for async route handlers. Return consistent error JSON shape.',status:'not_started',priority:'medium',due_date:futStr,category:'Development',tags:['backend','refactor'],sample,depends_on:[s.loginBug],annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.authRefactor,title:'Refactor auth middleware',description:'Extract JWT validation into reusable middleware. Add rate limiting.',status:'not_started',priority:'medium',due_date:nextWkStr,category:'Development',tags:['backend','security','refactor'],sample,depends_on:[s.errorHandling],annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.teamMetrics,title:'Compile team metrics',description:'Gather velocity, burndown, and cycle time data for the monthly stakeholder deck.',status:'not_started',priority:'medium',due_date:futStr,category:'Reports',tags:['metrics','team','analysis'],sample,recur:'monthly',depends_on:[s.weeklyReview],annotations:[{id:sid(),text:'Jira dashboard already configured — just need to export',timestamp:nowISO()}],created_at:lastWkStr+'T11:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.blogPost,title:'Write blog post on accessibility',description:'Draft a post about WCAG 2.2 compliance — what changed and how to audit.',status:'in_progress',priority:'low',due_date:nextWkStr,category:'Content',tags:['writing','blog','a11y'],sample,annotations:[{id:sid(),text:'Outline approved by marketing. Target: 1500 words.',timestamp:nowISO()}],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.grocery,title:'Grocery shopping',description:'Weekly run: veggies, eggs, milk, bread, oats.',status:'not_started',priority:'low',due_date:todayStr(),category:'Personal',tags:['errand','routine'],sample,recur:'weekly',annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.meditate,title:'Evening meditation',description:'10 min guided meditation using Headspace.',status:'done',priority:'medium',due_date:yestStr,start_date:yestStr,time:'21:00',category:'Health',tags:['routine','mindfulness'],sample,recur:'daily',annotations:[{id:sid(),text:'Session 45/90 on Headspace — focus pack',timestamp:nowISO()}],created_at:yestStr+'T21:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.garden,title:'Plant spring vegetables',description:'Prepare raised beds and sow seeds for tomatoes, peppers, basil.',status:'not_started',priority:'low',due_date:futStr,category:'Personal',tags:['garden','outdoor'],sample,annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.invoice,title:'Send Q2 invoices',description:'Generate and send invoices for Q2 consulting work. Follow up on overdue payments.',status:'not_started',priority:'high',due_date:futStr,category:'Finance',tags:['billing','client'],sample,depends_on:[s.budget],annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.portfolio,title:'Update portfolio site',description:'Add new projects and case studies. Update tech stack section with recent work.',status:'in_progress',priority:'medium',due_date:nextWkStr,start_date:futStr,category:'Design',tags:['design','frontend','portfolio'],sample,annotations:[{id:sid(),text:'Decided on Astro + Tailwind stack',timestamp:nowISO()}],created_at:lastWkStr+'T15:00:00',updated_at:nowISO(),deleted_at:null},
    {id:s.standup,title:'Daily standup',description:'Team sync — updates, blockers, priorities.',status:'not_started',priority:'medium',due_date:todayStr(),start_date:todayStr(),time:'09:00',category:'Meetings',tags:['recurring','team'],sample,recur:'daily',annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null},
    {id:s.podcast,title:'Record podcast episode',description:'Episode on "Building in public" — prep talking points and invite co-host.',status:'not_started',priority:'medium',due_date:nextWkStr,category:'Content',tags:['writing','podcast'],sample,depends_on:[s.blogPost],annotations:[],created_at:nowISO(),updated_at:nowISO(),deleted_at:null}
  ]
}
...
function removeSampleTasks(){
  ...
  const kept=tasks.filter(t=>!t.sample);
...
// ========== Home Page (Win95 desktop) ==========
function renderHomePage(){
  const page=document.getElementById('page-home');
  if(page.dataset.homeReady)return;
  ...
  // Theme toggle
  document.getElementById('themeSwitch').addEventListener('click',cycleTheme);

  // Status bar theme
  const st=document.getElementById('statusTheme');
  if(st){
    const t=document.documentElement.getAttribute('data-theme');
    st.textContent='Theme: '+(t==='nord-light'?'Nord Light':'Nord Dark');
  }
  ...
  function toggleMinimize(){
    const body=document.querySelector('#page-home .dialog-body');
  ...
  // Populate category select from existing tasks
  function populateCategories(){
    const sel=document.getElementById('taskCategory');
    const tasks=getTasks();
    const cats=[...new Set(tasks.map(t=>t.category).filter(Boolean))].sort();
    const current=sel.value;
    sel.innerHTML='<option value="">None</option>'+cats.map(c=>'<option value="'+c.replace(/"/g,'&quot;')+'">'+c.replace(/"/g,'&quot;')+'</option>').join('');
    sel.value=current||'';
  }
  populateCategories();

  // Advanced toggle — refresh categories on open
  document.getElementById('toggleAdvanced').addEventListener('click',function(){
    ...
    if(isOpen&&panel.classList.contains('closed'))populateCategories();
    if(!isOpen)populateCategories();
  });

  // Priority buttons
  document.querySelectorAll('#page-home .prio-btn').forEach(function(btn){
    btn.addEventListener('click',function(){
      document.querySelectorAll('#page-home .prio-btn').forEach(function(b){b.classList.remove('active')});
  ...
  document.getElementById('addBtn').addEventListener('click',function(){
    ...
    const advTitle=document.getElementById('taskTitle').value.trim();
    const finalTitle=raw||advTitle;
    if(!finalTitle){
    ...
    if(!parsed)parsed={title:finalTitle||'Untitled',priority:'medium',due_date:''};
    var t=buildTask({
      title:parsed.title,
    ...
    t.start_date=document.getElementById('taskStart').value||'';
    t.time=document.getElementById('taskTime').value||'';
    t.category=document.getElementById('taskCategory').value||'';
    var tags=document.getElementById('taskTags').value;
    t.tags=tags?tags.split(',').map(function(x){return x.trim()}).filter(Boolean):[];
    tasks.push(t);
    saveTasks(tasks);
    ...
    inp.value='';
    clearAdvancedFields();
    if(document.querySelector('#page-home .dialog-body').style.display!=='none'){
      toggleMinimize();
    }
  });

  function clearAdvancedFields(){
    ['taskTitle','taskDesc','taskDue','taskStart','taskTime','taskCategory','taskTags'].forEach(function(id){
  ...
  // Desktop icons
  document.querySelectorAll('#page-home .desk-icon').forEach(function(icon){
    icon.addEventListener('click',function(){
      document.querySelectorAll('#page-home .desk-icon').forEach(function(i){i.classList.remove('selected')});
  ...
  (function(){
    ...
    document.querySelector('#page-home .desktop').addEventListener('contextmenu',function(e){
      e.preventDefault();
      var x=Math.min(e.clientX,window.innerWidth-menu.offsetWidth-4);
      var y=Math.min(e.clientY,window.innerHeight-menu.offsetHeight-30);
